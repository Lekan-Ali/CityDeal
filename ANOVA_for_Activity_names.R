...
anova_fit_activity_name <- aov(Employed ~ ActivityName, data = df4) 

# model summary
summary(anova_fit_activity_name)


# plot anova fit
plotmeans(Employed ~ ActivityName, data = df4)


# calculate group means
activity_name_means <- aggregate(df4$Employed, by = list(df4$ActivityName)
                                  , FUN = mean)

# rename variable
names(activity_name_means)[1:2] <- c("Activity_group_name", "group_means")
...
df6 <- merge(df5, activity_name_means,by.x = "ActivityName", by.y = "Activity_group_name")

df7 <- df6 %>% filter(nvhActivityGroupName == "Support"
               | nvhActivityGroupName == "Job Related"
               | nvhActivityGroupName == "Job search and job matching")


# create a facet plot of activity group and activity name
ggplot(df7, aes(reorder(ActivityName,group_means), group_means)) +
  geom_col() +
  coord_flip() +
...
# create aesthetic
p1 <- ggplot(activity_name_means
             , aes(reorder(Activity_group_name,group_means)
                   , group_means))
